...
## `createStore<T>()` — Named App-Level Stores

The module-level pattern above works well, but all stores look the same: a collection of loose exported `useState()` values. `createStore()` provides a **named, discoverable** wrapper that:

- Registers the store in `globalThis.__NC_STORES__` for dev-tool inspection
- Prevents accidental duplicate initialisation (calling `createStore('cart', …)` twice returns the same store object)
- Makes the intent explicit — this is an app-level shared store, not page-local state

```typescript
// src/stores/cart.store.ts
import { createStore, getStore } from '@core/state.js';
...
// Registers once; subsequent imports reuse the same State<CartState> object
export const cartStore = createStore<CartState>('cart', { items: [], coupon: null });
```

```typescript
// In any controller or component:
import { getStore } from '@core/state.js';
import type { CartState } from '../stores/cart.store.js';

const cart = getStore<CartState>('cart');
if (cart) {
    cart.value = { ...cart.value, coupon: 'SUMMER10' };
}
```

`getStore(name)` returns `undefined` when called before `createStore` has run for that name — useful for lazy or conditional stores.

### Dev-tools visibility

All stores created with `createStore()` appear in `globalThis.__NC_STORES__` (a `Map<string, State<T>>`). The MetricsPanel and any custom dev-tool overlay can enumerate them:

```typescript
const stores = (globalThis as any).__NC_STORES__ as Map<string, unknown>;
stores.forEach((state, name) => console.log(name, state));
```

### `createStore` vs module-level `useState`

| | `createStore` | Module `useState` |
|---|---|---|
| Named / discoverable | ✅ | ❌ |
| Dev-tools visible | ✅ | ❌ |
| Deduplicated across imports | ✅ | ✅ (module cache) |
| Overhead | ~1 Map.set | None |
| Good for | App-level global state | Library/service internals |
