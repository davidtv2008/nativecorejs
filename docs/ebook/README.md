...
6. Use **`build:ssg`**, not `build:bots`.
7. Testing helpers: **`@testing/index.js`**.
8. Component Builder UI is **disabled** in shipped denc-tools.
...
| 16 | [16-wires.md](./16-wires.md) | Optional declarative `wire-*` bindings |
...
| 18 | [18-dev-tools.md](./18-dev-tools.md) | HMR, overlay, editor, outline (builder off) |
...
On Windows PowerShell, prefer `npm.cmd` so flags after `--` are preserved.
...
`bindAttr` as a Component method, `build:bots`, and multi-project mega-curricula.
