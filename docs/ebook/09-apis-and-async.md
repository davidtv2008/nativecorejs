# Chapter 09 — APIs and Async Data

> **What you'll build in this chapter:** Connect Taskflow's `tasksController` to the mock API with `api.getCached()`, add a loading skeleton and error state, and implement creating a new task with `api.post()`.
...
2. **`api.service`** — a project-level service generated by `create-nativecore` in `src/services/api.service.js`. It builds on top of `http`, layers in app-specific concerns (auth tokens, refresh-token handling, response caching with tags), and is the import you reach for in controllers.

This chapter covers both surfaces, then builds the Taskflow `projectsController` from scratch as a reference example. At the end you will apply the same pattern directly to Taskflow's `tasksController`.
...
const { data, error } = await http.safeGet('/projects', {
  retries: 2,
  backoff: 'exponential',
});
...
try {
  const project = await http.post('/projects', { name: 'Acme' });
} catch (err) {
...
// Project shape: { id, name, taskCount, status, updatedAt }

export async function projectsController() => void> {
  const scope     = dom.view('projects');
  ...
  errorMsg.subscribe(msg => {
    const alert = scope.$('.projects-error');
    alert.toggleAttribute('hidden', !msg);
    if (msg) alert.setAttribute('message', msg);
  });
...
> **Apply to Taskflow — update `tasks.controller.js`**
>
> The `projectsController` above is a complete reference example. Now apply the same pattern to Taskflow's `tasksController`:
>
> 1. Open `src/controllers/tasks.controller.js`.
> 2. In the **Data Fetch** section add:
>    ```javascript
>    const { data, error } = await api.getCached('/tasks', {
>      ttl:        60_000,
>      revalidate: true,
>      tags:       ['tasks'],
>    });
>    if (error) { errorMsg.value = error.message; return dispose; }
>    renderTasks(data);
>    ```
> 3. Wire `isLoading` and `errorMsg` to the loading skeleton and error alert in `tasks.html`.
> 4. In your create-task handler, after a successful `api.post('/tasks', body)`, call `api.invalidateTags(['tasks'])` so the next load fetches fresh data.
> 5. Navigate to `/tasks` and verify the task list populates from the mock API.
