...
export class NcNumberInput extends Component {
    ...
    _clamp(val) {
        ...
        if (min !== null && val < Number(min))
            return Number(min);
        if (max !== null && val > Number(max))
            return Number(max);
        return val;
    }
    _atMin() {
    ...
    _bindEvents() {
        ...
        const input = sr.querySelector('input[type="number"]');
        const decBtn = sr.querySelector('.btn-dec');
        const incBtn = sr.querySelector('.btn-inc');
        // Direct input
        ...
        // Keyboard on buttons
        decBtn.addEventListener('click', () => { if (decBtn.getAttribute('aria-disabled') !== 'true')
            this._step(-1); });
        incBtn.addEventListener('click', () => { if (incBtn.getAttribute('aria-disabled') !== 'true')
            this._step(1); });
        // Arrow keys on input
    ...
    _updateButtons(value) {
        ...
        const incBtn = sr.querySelector('.btn-inc');
        if (decBtn)
            decBtn.setAttribute('aria-disabled', String(min !== null && value <= Number(min)));
        if (incBtn)
            incBtn.setAttribute('aria-disabled', String(max !== null && value >= Number(max)));
    }
    onUnmount() {
        if (this._holdTimer)
            clearTimeout(this._holdTimer);
