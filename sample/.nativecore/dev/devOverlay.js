...
function runSeoScan() {
    ...
    const pushInput = (row) => {
        if (inputs.some(i => i.group === 'img' && i.imgSrcContains === row.imgSrcContains))
            return;
        if (row.group !== 'img' && inputs.some(i => i.key === row.key && i.group === row.group))
            return;
        inputs.push(row);
    };
    // 1. html[lang] — 5 pts
...
function patchConsole() {
    ...
    console.error = (...args) => {
        ...
        if (state.consoleLog.length > MAX_CONSOLE_LOG)
            state.consoleLog.pop();
        origError(...args);
    };
    console.warn = (...args) => {
        state.consoleWarns++;
...
function observeEffectLoopGuards() {
    window.addEventListener('nativecore:effect-loop-guard', (e) => {
        ...
        if (state.effectGuardLog.length > MAX_EFFECT_GUARD_LOG)
            state.effectGuardLog.pop();
    });
}
// ─── Fetch intercept ─────────────────────────────────────────────────────────
function patchFetch() {
    ...
    window.fetch = async (input, init) => {
        ...
        try {
            ...
            if (state.apiLog.length > MAX_API_LOG)
                state.apiLog.pop();
            if (!res.ok)
                state.netFailCount++;
            return res;
    ...
    try {
        longTaskObserver = new PerformanceObserver((list) => {
            for (const e of list.getEntries()) {
                ...
                if (state.longTaskLog.length > MAX_LONG_TASK_LOG)
                    state.longTaskLog.pop();
                if (state.longTaskWarningTimer)
                    clearTimeout(state.longTaskWarningTimer);
...
function observeRouteChanges() {
    ...
    window.addEventListener('pageloaded', () => {
        ...
        if (state.routeHistory.length > MAX_ROUTE_LOG)
...
function renderOverlayHTML() {
    ...
    if (state.fcp > 0 || state.lcp > 0) {
        if (state.fcp > 0)
            html += row('FCP', `<span style="color:${msColor(state.fcp, 1800, 3000)}">${formatMs(state.fcp)}</span>`, 'paint');
        if (state.lcp > 0)
            html += row('LCP', `<span style="color:${msColor(state.lcp, 2500, 4000)}">${formatMs(state.lcp)}</span>`, 'paint');
        html += `<hr class="nc-div">`;
    }
...
function bindOverlayEvents(el) {
    ...
    el.querySelectorAll('.nc-row[data-nc-section]').forEach(row => {
        row.addEventListener('click', (e) => {
            e.stopPropagation();
            const section = row.dataset.ncSection;
            openModal(section);
        });
    });
}
...
function modalDom() {
    ...
    return modalHeader('DOM — Node Count') +
        ...
        leakWarning +
        sect('What to watch for', '<div style="color:rgba(0,255,136,0.45);font-size:10px;line-height:1.8">' +
...
function modalPaint() {
    return modalHeader('Paint Timing — FCP / LCP') +
        sect('First Contentful Paint (FCP)', kv('Time', state.fcp > 0 ? `<span style="color:${msColor(state.fcp, 1800, 3000)}">${formatMs(state.fcp)}</span>` : '<span class="nc-muted">--</span>') +
        ...
        sect('Largest Contentful Paint (LCP)', kv('Time', state.lcp > 0 ? `<span style="color:${msColor(state.lcp, 2500, 4000)}">${formatMs(state.lcp)}</span>` : '<span class="nc-muted">--</span>') +
...
function modalRoutes() {
    const rows = state.routeHistory.length
        ? state.routeHistory.map(r => `<div class="nc-m-log-row" style="grid-template-columns:1fr auto auto">
                <span>${r.path}</span>
...
function modalLongTasks() {
    const rows = state.longTaskLog.length
        ? state.longTaskLog.map(t => `<div class="nc-m-log-row" style="grid-template-columns:1fr auto auto">
    ...
    return modalHeader('Long Tasks — Main Thread Blocking') +
        sect('Info', '<div style="color:rgba(0,255,136,0.45);font-size:10px;line-height:1.8">' +
...
function modalNet() {
    ...
    return modalHeader('Network — Fetch Log') +
        sect('Stats', kv('Pending fetches', state.pendingFetches > 0 ? `<span class="nc-caution">${state.pendingFetches}</span>` : '<span class="nc-muted">0</span>') +
...
function modalErrors() {
    const rows = state.consoleLog.length
        ? state.consoleLog.map(e => `<div class="nc-m-log-row ${e.level} " style="grid-template-columns:1fr auto">
    ...
    return modalHeader('Errors & Warnings') +
        sect('Summary', kv('Console errors', state.consoleErrors > 0 ? `<span class="nc-warn">${state.consoleErrors}</span>` : '<span class="nc-muted">0</span>') +
...
function modalEffectGuards() {
    const rows = state.effectGuardLog.length
        ? state.effectGuardLog.map(entry => `<div class="nc-m-log-row" style="grid-template-columns:1fr auto auto">
    ...
    return modalHeader('Effect Loop Guard') +
        sect('Summary', kv('Trips', state.effectGuardTrips > 0 ? `<span class="nc-warn">${state.effectGuardTrips}</span>` : '<span class="nc-muted">0</span>') +
        ...
        sect('What it means', '<div style="color:rgba(0,255,136,0.45);font-size:10px;line-height:1.8">' +
...
function modalConn() {
    ...
    return modalHeader('Connection — Network Info') +
        sect('Network Information API', kv('Effective type', `<span style="color:${typeColor}">${conn.effectiveType ?? '--'}</span>`) +
...
function makeDraggable(el) {
    ...
    el.addEventListener('mousedown', (e) => {
        const t = e.target;
        if (t.dataset.ncClose || t.classList.contains('nc-row'))
            return;
        dragging = true;
