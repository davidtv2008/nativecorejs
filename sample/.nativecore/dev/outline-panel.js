...
export class OutlinePanel {
    ...
    isDevToolElement(element) {
        ...
        if (element.hasAttribute('data-nc-dev') ||
            element.hasAttribute('data-denc-tool')) {
            return true;
        }
        return false;
    }
    ...
    buildTree(element, container, level = 0) {
        ...
        if (isCustom) {
            ...
            gear?.addEventListener('click', (e) => {
                e.stopPropagation();
                this.onEdit(element, true); // true = from outline tree
            });
        }
    ...
    expandToElement(targetElement) {
        ...
        while (elem && elem !== document.body) {
            ...
            if (parent) {
                const siblings = Array.from(parent.children).filter(child => child.tagName === elem.tagName);
