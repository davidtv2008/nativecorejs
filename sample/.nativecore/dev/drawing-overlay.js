...
function drawStroke(ctx, stroke) {
    ...
    if (stroke.tool === 'arrow') {
        ...
        ctx.beginPath();
        ctx.moveTo(x2, y2);
        ctx.lineTo(x2 - headLen * (ux * Math.cos(angle) - uy * Math.sin(angle)), y2 - headLen * (uy * Math.cos(angle) + ux * Math.sin(angle)));
        ctx.moveTo(x2, y2);
        ctx.lineTo(x2 - headLen * (ux * Math.cos(-angle) - uy * Math.sin(-angle)), y2 - headLen * (uy * Math.cos(-angle) + ux * Math.sin(-angle)));
        ctx.stroke();
    }
...
function hitTest(stroke, x, y) {
    ...
    if (stroke.tool === 'pen') {
        for (let i = 1; i < stroke.points.length; i++) {
            ...
            if (distPointToSegment(x, y, p.x, p.y, q.x, q.y) <= thresh)
                return true;
        }
        return false;
    }
...
function findHit(x, y) {
    for (let i = state.committed.length - 1; i >= 0; i--) {
        if (hitTest(state.committed[i], x, y))
            return i;
    }
    return -1;
}
...
function moveDraw(x, y) {
    if (state.tool === 'move') {
        if (!state.drawing || state.dragIndex < 0)
            return;
...
function renderToolbar(bar) {
    ...
    for (const t of TOOLS) {
        ...
        btn.addEventListener('click', () => {
            ...
            const canvas = getCanvas();
            if (canvas)
                canvas.classList.toggle('nc-cursor-move', t.id === 'move');
            renderToolbar(bar);
        });
        bar.appendChild(btn);
    }
...
export class DrawingOverlay {
    ...
    destroy() {
        ...
        if (navHandler)
            window.removeEventListener('pageloaded', navHandler);
