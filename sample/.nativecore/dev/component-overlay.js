...
export class ComponentOverlay {
    onEdit;
    observedComponents = new Set();
    componentButtons = new Map();
    ...
    isCustomComponent(element) {
        ...
        if (tagName.startsWith('nc-dev'))
            return false;
        if (tagName.startsWith('ion-') || tagName.startsWith('mat-'))
            return false;
        if (tagName.startsWith('nc-outline'))
            return false;
        if (tagName === 'nativecore-denc-indicator')
            return false;
        return customElements.get(tagName) !== undefined || element.shadowRoot !== null;
    }
    processElement(element) {
        if (!this.isCustomComponent(element))
            return;
        if (this.observedComponents.has(element))
            return;
    ...
    updateControlPosition(element, button) {
        if (!this.visible) {
