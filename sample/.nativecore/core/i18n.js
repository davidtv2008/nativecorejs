    ...
    async loadNamespace(namespace, locale = this.locale.value) {
        ...
        const task = (async () => {
            try {
                ...
                const set = this.loadedNamespaces.get(namespace) ?? new Set();
                set.add(locale);
...
function interpolate(message, params) {
    return message.replace(/\{(\w+)\}/g, (full, name) => Object.prototype.hasOwnProperty.call(params, name) ? String(params[name]) : full);
}
...
export function configureI18n(options) {
    if (options.messages)
        i18n.extend(options.messages);
    if (options.defaultLocale && !i18n.locale.value)
