...
export class NcInput extends Component {
    ...
    template() {
        ...
        return html`
            ...
            ${trusted(error ? `<span class="subtext subtext--error" id="subtext" role="alert">${error}</span>` : '')}
            ${trusted(hint && !error ? `<span class="subtext subtext--hint" id="subtext">${hint}</span>` : '')}
        `;
    }
