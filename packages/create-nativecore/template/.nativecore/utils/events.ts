...
export const onKeydown = (selectorOrElement: string | Element | null, handler: (event: KeyboardEvent) => void) =>
    on<KeyboardEvent>(selectorOrElement, 'keydown', handler);

export const onKeyup = (selectorOrElement: string | Element | null, handler: (event: KeyboardEvent) => void) =>
    on<KeyboardEvent>(selectorOrElement, 'keyup', handler);

export const onFocus = (selectorOrElement: string | Element | null, handler: (event: FocusEvent) => void) =>
    on<FocusEvent>(selectorOrElement, 'focus', handler);

export const onBlur = (selectorOrElement: string | Element | null, handler: (event: FocusEvent) => void) =>
    on<FocusEvent>(selectorOrElement, 'blur', handler);

// focusin/focusout bubble — useful for detecting focus on a container's children
export const onFocusin = (selectorOrElement: string | Element | null, handler: (event: FocusEvent) => void) =>
    on<FocusEvent>(selectorOrElement, 'focusin', handler);

export const onFocusout = (selectorOrElement: string | Element | null, handler: (event: FocusEvent) => void) =>
    on<FocusEvent>(selectorOrElement, 'focusout', handler);

export const onScroll = (selectorOrElement: string | Element | Window | null, handler: (event: Event) => void) =>
    on(selectorOrElement as string | Element | null, 'scroll', handler);

export const onMouseenter = (selectorOrElement: string | Element | null, handler: (event: MouseEvent) => void) =>
    on<MouseEvent>(selectorOrElement, 'mouseenter', handler);

export const onMouseleave = (selectorOrElement: string | Element | null, handler: (event: MouseEvent) => void) =>
    on<MouseEvent>(selectorOrElement, 'mouseleave', handler);

export const onDblclick = (selectorOrElement: string | Element | null, handler: (event: MouseEvent) => void) =>
    on<MouseEvent>(selectorOrElement, 'dblclick', handler);
...
export function trackEvents() {
    ...
    return {
        ...
        onKeydown(selectorOrElement: string | Element | null, handler: (event: KeyboardEvent) => void): void {
            cleanupFunctions.push(onKeydown(selectorOrElement, handler));
        },

        onKeyup(selectorOrElement: string | Element | null, handler: (event: KeyboardEvent) => void): void {
            cleanupFunctions.push(onKeyup(selectorOrElement, handler));
        },

        onFocus(selectorOrElement: string | Element | null, handler: (event: FocusEvent) => void): void {
            cleanupFunctions.push(onFocus(selectorOrElement, handler));
        },

        onBlur(selectorOrElement: string | Element | null, handler: (event: FocusEvent) => void): void {
            cleanupFunctions.push(onBlur(selectorOrElement, handler));
        },

        onFocusin(selectorOrElement: string | Element | null, handler: (event: FocusEvent) => void): void {
            cleanupFunctions.push(onFocusin(selectorOrElement, handler));
        },

        onFocusout(selectorOrElement: string | Element | null, handler: (event: FocusEvent) => void): void {
            cleanupFunctions.push(onFocusout(selectorOrElement, handler));
        },

        onScroll(selectorOrElement: string | Element | null, handler: (event: Event) => void): void {
            cleanupFunctions.push(onScroll(selectorOrElement, handler));
        },

        onMouseenter(selectorOrElement: string | Element | null, handler: (event: MouseEvent) => void): void {
            cleanupFunctions.push(onMouseenter(selectorOrElement, handler));
        },

        onMouseleave(selectorOrElement: string | Element | null, handler: (event: MouseEvent) => void): void {
            cleanupFunctions.push(onMouseleave(selectorOrElement, handler));
        },

        onDblclick(selectorOrElement: string | Element | null, handler: (event: MouseEvent) => void): void {
