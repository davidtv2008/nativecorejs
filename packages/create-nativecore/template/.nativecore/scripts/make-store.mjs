...
async function main() {
    ...
    const storeFile = path.join(storesDir, `${kebabName}.store.ts`);
    const indexFile = path.join(storesDir, 'index.ts');
    ...
    if (fs.existsSync(storeFile)) {
        console.error(`Error: Store "${kebabName}.store.ts" already exists`);
        rl.close();
        process.exit(1);
    }
...
// ── Module-level state ────────────────────────────────────────────────────────
// pausePageCleanupCollection prevents the router from cleaning these up on navigation.
...
/**
 * Load all ${titleName.toLowerCase()} items from the API.
...
/**
 * Add a new ${titleName.toLowerCase()} item optimistically, then confirm with the API.
 */
...
/**
 * Remove a ${titleName.toLowerCase()} item by id.
 */
...
`;
    ...
    console.log(`\n✓ Created store: src/stores/${kebabName}.store.ts`);
    ...
    if (fs.existsSync(indexFile)) {
        ...
        if (!indexContent.includes(exportLine)) {
            ...
            console.log(`✓ Exported from: src/stores/index.ts`);
        }
    } else {
        fs.writeFileSync(indexFile, `export * from './${kebabName}.store.js';\n`);
        console.log(`✓ Created barrel: src/stores/index.ts`);
    }

    console.log('\nNext steps:');
    console.log(`  1. Edit src/stores/${kebabName}.store.ts — add your types and fields`);
