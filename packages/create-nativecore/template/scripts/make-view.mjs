...
export async function ${controllerName}Controller(params: Record<string, string> = {}): Promise<() => void> {
  ...
  const data = dom.data('${flatName}');
  const titleElement = data.hook('title');
  const summaryElement = data.hook('summary');
  const primaryAction = data.component('primary-action');
  ...
  const titleText = computed(() => userState.value?.name
    ? '${viewTitle} for ' + userState.value.name
    : '${viewTitle}');
