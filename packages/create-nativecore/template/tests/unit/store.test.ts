...
// ── Inline minimal store (mirrors what make:store generates) ──────────────────
// In a real project this lives in src/stores/task.store.ts and you import it.
...
const items   = useState<Task[]>([]);
const loading = useState(false);
const error   = useState<string | null>(null);

const mockApi = {
    getCached: vi.fn<() => Promise<Task[]>>(),
    post:      vi.fn<() => Promise<Task>>(),
...
async function loadTasks(): Promise<void> {
    ...
    try {
        const data = await mockApi.getCached() as Task[];
...
async function addTask(task: Omit<Task, 'id'>): Promise<void> {
    const created = await mockApi.post() as Task;
...
describe('task store', () => {
    ...
    describe('addTask()', () => {
        it('appends the created task to items', async () => {
            items.value = [TASKS[0]];
            const newTask: Task = { id: '99', title: 'New task', status: 'todo' };
