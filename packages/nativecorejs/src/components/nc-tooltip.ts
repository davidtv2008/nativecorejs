...
export class NcTooltip extends CoreComponent {
    static useShadowDOM = true;
    static observedAttributes = ['tip', 'placement', 'delay', 'variant'];
    static attributeOptions = { placement: ['top', 'bottom', 'left', 'right'], variant: ['default', 'light'] };
    static attributeOrder   = ['tip', 'placement', 'delay', 'variant'];

    // -- Refs -----------------------------------------------------------------
    declare tipEl: HTMLDivElement;
    ...
    static styles = css`
        ...
        .tip {
            ...
            padding: 5px 10px; border-radius: var(--nc-radius-sm, 4px);
            ...
            max-width: 220px; overflow-wrap: break-word; white-space: normal; text-align: center;
        }
    ...
    template() {
        return `            <slot></slot>
            <div ref="tipEl" class="tip tip--default" data-placement="top" role="tooltip" aria-hidden="true"></div>
        `;
    }
    ...
    onMount() {
        ...
        this.on(this, 'mouseenter', show);
        this.on(this, 'mouseleave', hide);
        this.on(this, 'focusin',    show);
        this.on(this, 'focusout',   hide);
    ...
    private _syncFromAttrs() {
        ...
        this.tipEl.textContent = this.getAttribute('tip') || '';
        this.tipEl.dataset.placement = placement;
        this.tipEl.className = `tip tip--${variant}`;
        if (this._visible) this.tipEl.classList.add('visible');
    }
}

if (!customElements.get('nc-tooltip')) customElements.define('nc-tooltip', NcTooltip);
