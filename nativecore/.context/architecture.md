...
| templates.ts    | `html``, `css``, `trusted()`, `escapeHtml()`, `sanitizeURL()`                    |
| cacheBuster.ts  | `bustCache(path)`, `cacheVersion`, `importWithBust(path)`                       |
| wires.ts        | `wireContents()`, `wireInputs()`, `wireAttributes()` — declarative view binding |

#### Declarative View Binding (`@core-utils/wires.js`)

Always import all three from the unified barrel — never from individual files:

```typescript
import { wireContents, wireInputs, wireAttributes } from '@core-utils/wires.js';
```

| Utility            | HTML attribute                    | Direction             | Use for                        |
|--------------------|-----------------------------------|-----------------------|--------------------------------|
| `wireContents()`   | `wire-content="key"`              | state → textContent   | headings, paragraphs, spans    |
| `wireInputs()`     | `wire-input="key"`                | state ↔ input value   | input, select, textarea        |
| `wireAttributes()` | `wire-attribute="key:attr-name"`  | state → setAttribute  | data-*, aria-*, src, disabled  |

All three auto-register cleanup with `PageCleanupRegistry` — no manual cleanup needed in controllers.
In components use `this.wireContents()`, `this.wireInputs()`, `this.wireAttributes()` from `onMount()`.
