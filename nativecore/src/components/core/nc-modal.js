...
class NcModal extends Component {
  ...
  static open(id) {
    dom.query(`#${id}`)?.setAttribute("open", "");
  }
  static close(id) {
    dom.query(`#${id}`)?.removeAttribute("open");
  }
  _onKeydown = null;
  template() {
  ...
  _bindEvents() {
    ...
    overlay.addEventListener("click", (e) => {
      ...
      if (!dialog.contains(e.target)) this._close();
    });
    this._onKeydown = (e) => {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    if (name === "open" && this._mounted) {
      ...
      if (dialog) {
        dialog.style.transform = open ? "translateY(0) scale(1)" : "translateY(12px) scale(0.97)";
