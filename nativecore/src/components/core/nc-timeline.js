...
class NcTimelineItem extends Component {
  static useShadowDOM = true;
  static get observedAttributes() {
    return ["color", "status", "active", "no-line"];
  }
  template() {
    ...
    const color = this.getAttribute("color") ?? STATUS_COLORS[status] ?? "var(--nc-primary)";
    const dotContent = SMALL_ICONS[iconKey] ? iconSvg(SMALL_ICONS[iconKey], 10) : status === "completed" ? iconSvg(SMALL_ICONS.check, 10) : "";
...
class NcTimeline extends Component {
  static useShadowDOM = true;
  template() {
    return `
            <style>
