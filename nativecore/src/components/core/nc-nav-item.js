...
class NcNavItem extends Component {
  static useShadowDOM = true;
  static get observedAttributes() {
    return ["active", "disabled", "badge", "href"];
  }
  template() {
    ...
    const iconHtml = NAV_ICONS[iconName] ? svgWrap(NAV_ICONS[iconName]) : iconName.startsWith("<") ? iconName : "";
    const badgeColors = {
    ...
    const tagAttrs = href ? `href="${href}" target="${target}"` : `type="button"`;
    return html`
            <style>
                :host { display: block; }
                ${tag} {
