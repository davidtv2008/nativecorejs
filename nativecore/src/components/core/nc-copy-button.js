...
class NcCopyButton extends Component {
  ...
  template() {
    ...
    const color = isCopied ? "var(--nc-success)" : variant === "primary" ? "var(--nc-white)" : "var(--nc-text)";
    return html`
            <style>
            ...
            <button type="button" aria-label="${isCopied ? copiedLabel : label}" ${isCopied ? "disabled" : ""}>
                ...
                ${trusted(!iconOnly ? `<span>${escapeHtml(isCopied ? copiedLabel : label)}</span>` : "")}
            </button>
        `;
  }
  onMount() {
