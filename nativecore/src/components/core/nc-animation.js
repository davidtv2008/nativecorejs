...
const _injectedKeyframes = /* @__PURE__ */ new Set();
class NcAnimation extends Component {
  ...
  _runParticles(preset) {
    ...
    const cfg = {
      ...
      speed: preset.particle.speed ? { min: preset.particle.speed.min * spreadScale, max: preset.particle.speed.max * spreadScale } : { min: 40, max: 120 }
    };
  ...
  _runCanvas2D(canvas, config, ox, oy, tx, ty, preset) {
    ...
    const drawParticle = (p) => {
      ...
      if (p.shape === "rect" && p.rot !== void 0) {
        ...
        const maxR = type === "explosion" ? Math.max(W, H) * 0.35 : Math.max(W, H) * 0.45;
    ...
    const loop = createAnimationLoop((dt) => {
      ...
      let alive = 0;
      const fadeRate = type === "ripple" ? 0.3 : type === "fire" ? 0.7 : type === "float" ? 0.15 : type === "firework" ? 0.42 : type === "explosion" ? 0.65 : 0.35;
      for (const p of particles) {
        p.vx += p.ax * dt;
        p.vy += p.ay * dt;
        p.x += p.vx * dt;
        p.y += p.vy * dt;
        if (p.rot !== void 0 && p.rotV !== void 0) p.rot += p.rotV * dt;
        if (type !== "ripple") {
