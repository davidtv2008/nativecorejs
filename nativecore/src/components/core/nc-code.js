...
const KEYWORDS_JS = /* @__PURE__ */ new Set(["const", "let", "var", "function", "return", "import", "export", "default", "class", "extends", "new", "this", "super", "if", "else", "for", "while", "do", "switch", "case", "break", "continue", "try", "catch", "finally", "throw", "typeof", "instanceof", "in", "of", "async", "await", "yield", "static", "get", "set", "null", "undefined", "true", "false", "void", "delete"]);
const KEYWORDS_TS = /* @__PURE__ */ new Set([...KEYWORDS_JS, "type", "interface", "enum", "namespace", "declare", "abstract", "as", "from", "implements", "keyof", "readonly", "infer", "never", "unknown", "any", "string", "number", "boolean", "object", "symbol", "bigint"]);
function escHtml(s) {
...
function tokenizeJson(code) {
  return escHtml(code).replace(/("(?:[^"\\]|\\.)*")\s*:/g, '<span class="t-attr">$1</span>:').replace(/:\s*("(?:[^"\\]|\\.)*")/g, ': <span class="t-string">$1</span>').replace(/:\s*(-?[0-9]+(?:\.[0-9]+)?)/g, ': <span class="t-number">$1</span>').replace(/:\s*(true|false|null)/g, ': <span class="t-keyword">$1</span>');
}
function tokenizeHtml(code) {
  return escHtml(code).replace(/&lt;!--[\s\S]*?--&gt;/g, (m) => `<span class="t-comment">${m}</span>`).replace(/(&lt;\/?)([\w:-]+)/g, (_, lt, tag) => `${lt}<span class="t-keyword">${tag}</span>`).replace(/([\w:-]+)=(&quot;[^&]*&quot;)/g, `<span class="t-attr">$1</span>=<span class="t-string">$2</span>`);
}
function tokenizeCss(code) {
  return escHtml(code).replace(/(\/\*[\s\S]*?\*\/)/g, '<span class="t-comment">$1</span>').replace(/(#[0-9a-fA-F]{3,8})/g, '<span class="t-number">$1</span>').replace(/([.#]?[\w:-]+)\s*{/g, '<span class="t-class">$1</span> {').replace(/([\w-]+)\s*:/g, '<span class="t-attr">$1</span>:');
}
class NcCode extends Component {
  ...
  _copy(btn) {
    const raw = this._code ?? this.$("#code")?.textContent?.replace(/\u200B/g, "") ?? "";
