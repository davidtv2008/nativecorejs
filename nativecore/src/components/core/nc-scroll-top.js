...
class NcScrollTop extends Component {
  ...
  template() {
    ...
    const posStyle = pos === "bottom-left" ? `left:${offset}px;right:auto;` : pos === "bottom-center" ? `left:50%;transform:translateX(-50%);` : `right:${offset}px;left:auto;`;
    return html`
            <style>
  ...
  onMount() {
    ...
    this._scrollTarget = targetSelector ? dom.query(targetSelector) ?? window : window;
    const updateVisibility = () => {
      const currentScroll = this._scrollTarget instanceof Window ? this._scrollTarget.scrollY : this._scrollTarget.scrollTop;
