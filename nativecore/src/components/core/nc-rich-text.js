...
class NcRichText extends Component {
  ...
  _onContentChange() {
    const editable = this.$(".editable");
    const html2 = editable.innerHTML;
    this._value = html2;
    const hidden = this.$('input[type="hidden"]');
    if (hidden) hidden.value = html2;
    const name = this.getAttribute("name") || "";
    this.dispatchEvent(new CustomEvent("input", {
      bubbles: true,
      composed: true,
      detail: { value: html2, name }
    }));
  }
  _updateToolbarState() {
  ...
  setValue(html2) {
    this._value = html2;
    const editable = this.$(".editable");
    if (editable) editable.innerHTML = html2;
    const hidden = this.$('input[type="hidden"]');
    if (hidden) hidden.value = html2;
  }
