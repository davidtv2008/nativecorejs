...
class NcTagInput extends Component {
  ...
  template() {
    ...
    const tagsHtml = this._tags.map((tag, i) => `
            <span class="tag" data-index="${i}">
                ...
                ${!disabled && !readonly ? `<button class="tag-remove" type="button" data-index="${i}" aria-label="Remove ${this._escape(tag)}">
