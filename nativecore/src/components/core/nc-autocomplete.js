...
class NcAutocomplete extends Component {
  ...
  _filtered() {
    ...
    return this._getOptions().filter((o) => o.toLowerCase().includes(query)).slice(0, max);
  }
  template() {
    if (!this._mounted) {
    ...
    return html`
      ...
      const hl = escapedQuery ? safeOpt.replace(new RegExp(`(${escapedQuery})`, "gi"), "<mark>$1</mark>") : safeOpt;
