...
class NcTabs extends Component {
  ...
  onMount() {
    ...
    this._onShadowKeydown = (e) => {
      ...
      while (tabs[next]?.hasAttribute("disabled") && attempts < len) {
        next = ke.key === "ArrowLeft" ? (next - 1 + len) % len : (next + 1) % len;
        attempts++;
      }
  ...
  _setupScrollArrows() {
    ...
    if (!this._scrollListenerSet) {
      ...
      this.$(".nc-tabs__scroll-btn--prev")?.addEventListener("click", () => {
        bar.scrollBy({ left: -150, behavior: "smooth" });
      });
      this.$(".nc-tabs__scroll-btn--next")?.addEventListener("click", () => {
        bar.scrollBy({ left: 150, behavior: "smooth" });
      });
    ...
    requestAnimationFrame(() => {
      this.shadowRoot?.querySelector(".nc-tabs__btn--active")?.scrollIntoView({ block: "nearest", inline: "nearest" });
    });
  }
  _updateScrollBtns() {
    const bar = this.$(".nc-tabs__bar");
