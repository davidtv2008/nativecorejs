...
class ApiService {
  ...
  queryKeyIndex = /* @__PURE__ */ new Map();
  ...
  invalidateQuery(queryKey, options = {}) {
    ...
    for (const [serializedQueryKey, cacheKeys] of this.queryKeyIndex.entries()) {
      const matches = options.exact ? serializedQueryKey === target : serializedQueryKey === target || serializedQueryKey.startsWith(`${target}|`);
      if (!matches) {
        continue;
      }
  ...
  async getCached(endpoint, options) {
    ...
    const now = Date.now();
    const ttlMs = Math.max(options.ttl, 0) * 1e3;
    ...
    const requestPromise = this.get(endpoint, options.params ?? {}).then((data) => {
  ...
  buildCacheKey(endpoint, options) {
    ...
    const queryString = new URLSearchParams(
      Object.entries(options.params ?? {}).reduce((accumulator, [key, value]) => {
  ...
  refreshCachedRequest(cacheKey, endpoint, options) {
    ...
    const refreshPromise = this.get(endpoint, options.params ?? {}).then((data) => {
      this.setCacheEntry(cacheKey, {
        data,
        cachedAt: Date.now(),
        ttl: options.ttl,
  ...
  setCacheEntry(cacheKey, entry) {
    ...
    if (entry.queryKey) {
      ...
      const existingKeys = this.queryKeyIndex.get(serializedQueryKey) ?? /* @__PURE__ */ new Set();
      existingKeys.add(cacheKey);
      this.queryKeyIndex.set(serializedQueryKey, existingKeys);
    }
    for (const tag of entry.tags) {
      const existingKeys = this.tagIndex.get(tag) ?? /* @__PURE__ */ new Set();
      existingKeys.add(cacheKey);
...
var stdin_default = new ApiService();
export {
