...
class ComponentEditor {
            ...
            <div class="nc-editor-drawer" id="ncEditorDrawer">
                <div class="nc-editor-header">
                    ...
                    <div class="nc-editor-header-actions">
                        <button class="nc-editor-icon-btn" id="ncEditorCopy" title="Copy Component Values">\u{1F4CB}</button>
                        <button class="nc-editor-icon-btn" id="ncEditorPaste" title="Paste Component Values">\u{1F4C4}</button>
                        <button class="nc-editor-icon-btn" id="ncEditorReset" title="Reset to Default">\u21BA</button>
                        <button class="nc-editor-icon-btn nc-editor-btn-danger" id="ncEditorDelete" title="Delete Instance">\u{1F5D1}\uFE0F</button>
    ...
    modal.innerHTML = `
            <div class="nc-editor-modal-content">
                <div class="nc-editor-modal-header">
                    <div class="nc-editor-modal-icon">\u{1F5D1}\uFE0F</div>
  ...
  buildDynamicContent(element, meta) {
    ...
    if (shadowRoot) {
      const nested = Array.from(shadowRoot.querySelectorAll("*")).filter((el) => el.tagName.includes("-") && customElements.get(el.tagName.toLowerCase())).reduce((acc, el) => {
  ...
  buildAttributeField(element, attr) {
    ...
    if (attr.type === "variant" && attr.variantOptions && attr.variantOptions.length > 0) {
      ...
      const options = attr.variantOptions.map(
        (opt) => `<option value="${opt}" ${opt === currentValue ? "selected" : ""}>${opt}</option>`
      ).join("");
      return `<div class="nc-editor-field">
                <label class="nc-editor-label">${attr.name}</label>
      ...
      input.addEventListener(eventType, (e) => {
        ...
        if (Object.keys(conditions).some((key) => key !== attrName)) {
          if (attrName === "layout" || Object.values(conditions).some((fn) => {
            ...
            const contentEl = this.drawer?.querySelector("#ncEditorContent");
            const sectionStates = /* @__PURE__ */ new Map();
            if (contentEl) {
            ...
            setTimeout(() => {
              if (this.currentElement && this.currentMetadata) {
                const contentEl2 = this.drawer?.querySelector("#ncEditorContent");
                if (contentEl2) {
                  contentEl2.innerHTML = this.buildDynamicContent(this.currentElement, this.currentMetadata);
                  this.attachListeners(contentEl2, this.currentElement);
                  contentEl2.querySelectorAll(".nc-editor-section").forEach((section) => {
                    const title = section.querySelector(".nc-editor-section-title")?.textContent || "";
