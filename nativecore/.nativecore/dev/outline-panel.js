class OutlinePanel {
  ...
  constructor(onEdit) {
    ...
    this.injectStyles();
    this.createPanel();
    document.addEventListener("nc-current-element-response", (e) => {
   ...
   */
  refreshStorageContent(container, storageObj) {
    container.innerHTML = "";
    const items = [];
  ...
  togglePanel() {
    ...
    if (this.isOpen) {
      ...
      if (storageContent) {
        this.refreshStorageContent(storageContent, sessionStorage);
      }
      const localStorageContent = this.panel?.querySelector("#localStorageContent");
      if (localStorageContent) {
        this.refreshStorageContent(localStorageContent, localStorage);
      }
      setTimeout(() => {
  ...
  refreshTree() {
    ...
    if (!tree) return;
    tree.innerHTML = "";
    this.buildTree(document.body, tree, -1);
  }
  /**
  ...
  isDevToolElement(element) {
    ...
    if (tagName?.startsWith("nc-dev") || tagName?.startsWith("nc-outline") || tagName?.startsWith("nc-editor") || tagName === "hmr-indicator" || tagName === "nativecore-denc-indicator") {
      return true;
    }
    if (classList?.contains("nc-denc-overlay") || classList?.contains("nc-editor-overlay") || classList?.contains("nc-editor-modal") || classList?.contains("nc-context-menu") || classList?.contains("nc-component-overlay") || classList?.contains("nc-outline-panel") || classList?.contains("nc-outline-tab") || classList?.contains("hmr-indicator") || classList?.contains("nativecore-denc-indicator")) {
      return true;
    }
    if (id?.includes("nc-dev") || id?.includes("nc-editor") || id?.includes("nc-outline") || id?.includes("hmr-") || id?.includes("nativecore-dev") || id?.includes("denc-indicator")) {
      return true;
    }
    if (element.hasAttribute("data-nc-dev") || element.hasAttribute("data-denc-tool")) {
      return true;
    }
    return false;
  }
   ...
   */
  buildTree(element, container, level = 0) {
    ...
    const isCustom = this.isCustomComponent(element);
    const hasChildren = Array.from(element.children).some((child) => {
      const childTag = child.tagName.toLowerCase();
      return !["script", "style", "noscript", "link", "meta"].includes(childTag) && !this.isDevToolElement(child);
    });
    ...
    if (hasChildren) {
      ...
      if (toggle) {
        ...
        nodeDiv.addEventListener("click", (e) => {
          if (e.target.closest(".nc-tree-gear")) {
            return;
          }
          e.stopPropagation();
    ...
    if (!hasChildren) {
      content.addEventListener("click", (e) => {
        ...
        element.style.outline = "2px solid #5cc8ff";
  ...
  adjustPanelWidth() {
    ...
    visibleNodes.forEach((node) => {
      ...
      if (isVisible) {
        const paddingLeft = parseInt(node.style.paddingLeft || "0");
  ...
  isCustomComponent(element) {
    const tagName = element.tagName.toLowerCase();
    return tagName.includes("-") && !tagName.startsWith("nc-dev") && !tagName.startsWith("ion-") && !tagName.startsWith("mat-");
  }
  /**
  ...
  setupMutationObserver() {
    this.mutationObserver = new MutationObserver((mutations) => {
      const isOutlineChange = mutations.some((mutation) => {
        ...
        return target.closest(".nc-outline-panel") || target.classList?.contains("nc-outline-panel") || target.classList?.contains("nc-outline-tab");
      });
      ...
      treeNodes.forEach((node) => {
        ...
        if (matches) {
          ...
          if (parent === targetElement) {
            node.style.backgroundColor = "rgba(92, 200, 255, 0.2)";
