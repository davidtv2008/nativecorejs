...
class DevTools {
  static TOGGLE_STORAGE_KEY = "nativecore-devtools-visible";
  overlay = null;
  editor = null;
  outlinePanel = null;
  drawingOverlay = null;
  ...
  isDevEnvironment() {
    ...
    const hasDevFlag = window.__NATIVECORE_DEV__ === true;
    return isLocalhost || hasDevFlag;
  }
  /**
   * Handle edit request from overlay
   */
  async onEditComponent(element, fromOutlineTree = false) {
   ...
   */
  async fetchComponentMetadata(tagName) {
  ...
  addDevIndicator() {
    ...
    indicator.innerHTML = `
            ...
            <button id="nativecore-denc-brush-btn" type="button" title="Toggle annotation drawing mode">\u{1F58C}\uFE0F</button>
        `;
    indicator.classList.toggle("is-off", !this.overlayVisible);
    indicator.addEventListener("click", () => this.toggleOverlayVisibility());
    const brushBtn = indicator.querySelector("#nativecore-denc-brush-btn");
  ...
  loadOverlayVisibilityPreference() {
    try {
  ...
  destroy() {
    ...
    const indicator = window.dom?.query?.("#nativecore-denc-indicator") || document.getElementById("nativecore-denc-indicator");
    if (indicator) {
      indicator.remove();
    }
    this.indicator = null;
