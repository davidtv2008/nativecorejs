...
class Router {
  ...
  _groupMiddlewares = [];
  _groupPrefix = "";
  _routeMiddlewares = /* @__PURE__ */ new Map();
  constructor() {
  ...
  getPathsForMiddleware(middlewareName) {
    return Array.from(this._routeMiddlewares.entries()).filter(([, tags]) => tags.includes(middlewareName)).map(([path]) => path);
  }
  /**
  ...
  async loadPage(route, state = {}, previousRoute = null) {
    ...
    try {
      const isPrerenderedInitialRoute = previousRoute === null && mainContent.getAttribute("data-prerendered-route") === route.path && !route.config.layout;
      if (progressBar) {
  ...
  async fetchHTML(file, policy, allowPrefetchCache = false) {
    ...
    const now = Date.now();
    const ttlMs = this.resolveCacheTtl(policy, entry) * 1e3;
  ...
  setQuery(patch, options = {}) {
    ...
    for (const [key, value] of Object.entries(patch)) {
      if (value === null || value === void 0) params.delete(key);
  ...
  async resolveContentTarget(mainContent, route) {
    ...
    const needsLayoutRender = this.renderedLayoutPath !== layoutRoute.path || !mainContent.querySelector("#route-outlet");
    ...
    if (!outlet) {
      console.error(
        `[router] Layout route "${layoutRoute.path}" is missing a #route-outlet element. Falling back to rendering into the layout root. Add <div id="route-outlet"></div> to ${layoutRoute.config.htmlFile} to silence this warning.`
      );
      this.renderedLayoutPath = null;
      return mainContent;
    }
