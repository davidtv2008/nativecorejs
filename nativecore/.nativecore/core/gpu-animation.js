function setGPUTransform(element, x, y, z = 0, scale = 1, rotate = 0) {
  element.style.transform = `translate3d(${x}px, ${y}px, ${z}px) scale(${scale}) rotate(${rotate}deg)`;
}
function setTransformVars(element, vars) {
  for (const [key, value] of Object.entries(vars)) {
    element.style.setProperty(`--${key}`, typeof value === "number" ? `${value}px` : value);
  }
}
function prepareForAnimation(element, properties = ["transform", "opacity"]) {
...
function slideIn(element, direction = "up", distance = 40, duration = 400) {
...
function createAnimationLoop(callback) {
  ...
  const tick = (currentTime) => {
    ...
    const deltaTime = Math.min((currentTime - lastTime) / 1e3, 0.1);
...
function createWebGLParticleSystem(canvas, config) {
...
const injectedAnimations = /* @__PURE__ */ new Set();
...
function injectCommonAnimations() {
